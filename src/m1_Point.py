...
class Point(object):
    ...
    def move_to(self, x, y):
        self.dtravel += self.get_distance_from(Point(x, y))
        self.x = x
        self.y = y
        self.moves += 1

    def move_by(self, dx, dy):
        self.dtravel += self.get_distance_from(Point(self.x+dx, self.y+dy))
        self.x += dx
        self.y += dy
        self.moves += 1
    ...
    def get_distance_from(self, origin):
    ...
    def halfway_to(self, pt):
        hx = self.x - (self.x - pt.x) / 2
        hy = self.y - (self.y - pt.y) / 2
        return Point(hx, hy)
